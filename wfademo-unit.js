...
async function notify(dst, txt) {
  const client = await MongoClient.connect(mongoURL, { useNewUrlParser: true }).catch(err => {console.error("Mongo Client Connect error", err)})
  if (!client) {
    return;
  }
  try {
...
async function add(cell) {
  const client = await MongoClient.connect(mongoURL).catch(err => {console.error("Mongo Client Connect error", err)})
  if (!client) {
    return;
  }
  try {
